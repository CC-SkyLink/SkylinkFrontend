...
import { BarChart3, Search, ScrollText } from "lucide-react";
...
import type { ReportType, DateRange, ActivityLog } from "./__components/types";
    ...
    <AdminLayout>
      <div className="space-y-6 text-left">
        ...
        <div className="flex gap-6 border-b border-slate-100">
          <button onClick={() => navigate(ROUTES.ADMIN_REPORTS)} className={cn("flex items-center gap-2 pb-3 text-sm font-bold border-b-2 transition-all cursor-pointer", activeTab === "reports" ? "text-[#496B92] border-[#496B92]" : "text-slate-400 border-transparent hover:text-slate-600")}>
            <BarChart3 size={18} /> Reports
          </button>
          <button onClick={() => navigate("/admin/activity-log")} className={cn("flex items-center gap-2 pb-3 text-sm font-bold border-b-2 transition-all cursor-pointer", activeTab === "activity-log" ? "text-[#496B92] border-[#496B92]" : "text-slate-400 border-transparent hover:text-slate-600")}>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
              <div className="relative col-span-1">
                ...
                <input type="text" placeholder="Search by details or ID..." value={logSearch} onChange={(e) => { setLogSearch(e.target.value); setLogPage(1); }} className="h-11 w-full rounded-xl bg-slate-50 pl-10 pr-4 text-sm outline-none border border-transparent focus:border-[#496B92]/20 focus:bg-white focus:ring-2 focus:ring-[#496B92]/10 transition-all placeholder:text-slate-400" />
              </div>
              <select value={logActionFilter} onChange={(e) => { setLogActionFilter(e.target.value); setLogPage(1); }} className="h-11 rounded-xl bg-slate-50 border border-transparent focus:border-[#496B92]/20 focus:bg-white focus:ring-2 focus:ring-[#496B92]/10 px-4 text-sm font-medium text-slate-600 outline-none transition-all cursor-pointer">
              ...
              <input type="date" value={logDateFilter} onChange={(e) => { setLogDateFilter(e.target.value); setLogPage(1); }} className="h-11 rounded-xl bg-slate-50 border border-transparent focus:border-[#496B92]/20 focus:bg-white focus:ring-2 focus:ring-[#496B92]/10 px-4 text-sm font-medium text-slate-600 outline-none transition-all cursor-pointer" />
            </div>
            <section className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="p-1">
                <DataTable columns={activityLogColumns} rows={paginatedLogs} rowKey={(r, idx) => r.timestamp + idx}
