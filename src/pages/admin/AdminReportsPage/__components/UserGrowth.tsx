...
const UserGrowth = ({ dateRangeLabel, onToast }: Props) => {
  const columns: TableColumn<ReportDataRow>[] = [
    ...
    { key: "value", header: "USERS", cell: (row) => <span className="font-bold text-slate-900">{(row.value as number).toLocaleString()}</span> },
    {
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-50">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => exportCSV("growth", EMPTY, "User Growth", (msg) => onToast(msg, "success"))}
            ...
            <button
              onClick={() => exportPDF("growth", EMPTY, "User Growth", dateRangeLabel, (msg) => onToast(msg, "info"), (msg) => onToast(msg, "success"))}
        ...
        <div className="py-16 text-center text-slate-400 font-medium text-sm">
          User growth analytics coming soon.
        </div>
      </div>
